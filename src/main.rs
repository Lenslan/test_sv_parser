...
fn extract_nonansi_ports(node: &RefNode, syntax_tree: &SyntaxTree) {
    ...
    for item in unwrap_node!(node, ModuleItem).into_iter().flatten() {
        ...
        if let Some(port_declaration) = unwrap_node!(item, PortDeclaration) {
            let mut direction_str = "inout".to_string();
            let mut width_str = "1-bit".to_string();

            // 从 Input/Output/InoutDeclaration 中确定方向
            if let Some(_) = unwrap_node!(port_declaration, InputDeclaration) {
                direction_str = "input".to_string();
            } else if let Some(_) = unwrap_node!(port_declaration, OutputDeclaration) {
                direction_str = "output".to_string();
            } else if let Some(_) = unwrap_node!(port_declaration, InoutDeclaration) {
                direction_str = "inout".to_string();
            }

            // 提取位宽
            if let Some(packed_dimension) = unwrap_node!(port_declaration, PackedDimension) {
                width_str = get_node_string(packed_dimension, syntax_tree);
            }
